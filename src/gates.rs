...
pub fn hadamard() -> Array {
    let coef: [Complex<f32>; 4] = [
        Complex::new(0.70710678118f32, 0.0),
        Complex::new(0.70710678118f32, 0.0),
        Complex::new(0.70710678118f32, 0.0),
        Complex::new(-0.70710678118f32, 0.0),
    ];
...
pub fn generate_gate(gate: Array, num_qubits: usize, target: i32) -> Array {
    ...
    if qubits_before == 0 && qubits_after == 0 {
        ...
        let after_arr = identity_t(
            Dim4::new(&[2 << (qubits_after - 1), 2 << (qubits_after - 1), 1, 1]),
            DType::C32,
        );
        return kron(&gate, &after_arr);
    } else if qubits_after == 0 {
        let before_arr = identity_t(
            Dim4::new(&[2 << (qubits_before - 1), 2 << (qubits_before - 1), 1, 1]),
            DType::C32,
        );
...
pub fn generate_cnot(num_qubits: usize, control: i32, target: i32) -> Array {
    let mut arr = constant(
        Complex::new(0.0f32, 0.0),
        Dim4::new(&[
            2u64.pow(num_qubits as u32),
            2u64.pow(num_qubits as u32),
            1,
            1,
        ]),
    ...
    for i in 0..(2u64.pow(num_qubits as u32) as i32) {
        if get_bit(i, c) == 1 {
            ...
            let position = &[
                Seq::new(i, i, 1),
                Seq::new(toggle_bit(i, t), toggle_bit(i, t), 1),
            ];
